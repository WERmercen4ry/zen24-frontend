...
const UpdatePackages = () => {
  ...
  const fetchPackageById = async () => {
    const res = await authorizedAxiosinstance.get(
      `${API_ROOT}dashboards/getPackageByID?packageId=${packageId}`
    );
  ...
  const updatePackage = async (e) => {
    ...
    try {
      ...
      const res = await authorizedAxiosinstance.put(
        `${API_ROOT}dashboards/updatePackage?packageId=${packageId}`,
        reqBody
      );
      ...
      if (res?.status === 200) {
        showToast(
          "Thông báo",
          "Chỉnh sửa gói tập thành công",
          TOAST_TYPES.SUCCESS
        );
      }
